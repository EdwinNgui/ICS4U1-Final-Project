 ...
 */
 ...
 public class Main {
         ...
         for (int k = 0; k < 2; k++) { // 15 rounds
             ...
             if (numOfPlayers >= 2) {
                 if (playersAvailable != 1) {
                     ...
                     playersAvailable = playTurn(p1, board, p2, p3, p4, chanceArr, playersAvailable);
                     p1.sortOwnedSpace(p1.getOwnedSpace(), p1.getOwnedSpace().length); // Sort requires player's array
                                                                                       // and array size
                 }
                 if (playersAvailable != 1) {
                     displayBoard(board, p2);
                     playersAvailable = playTurn(p2, board, p1, p3, p4, chanceArr, playersAvailable);
     ...
     public static int playTurn(Player player, boardSpace[][] board, Player playerA, Player playerB, Player playerC,
             Queue chanceArr, int playersAvailable) {
         int num = rollDice();
         ...
         if (player.getInGame()) { // If in game, continue
             ...
             do {
                 ...
                 numAns = input.nextInt();
                 input.nextLine();
                 if (numAns != 1 && numAns != 2 && numAns != 3 && numAns != 4) {
                     invalidInput = true;
                 }
      ...
      * Post: Returns nothing to main
      * Desc: Given text, it will print it out slowly; generally for human spoken dialogue
      */
